...
export function getAccount(req, response): void {
   if (!req.body) {
      response.sendStatus(400);
      // } else if (3 === 3) {
      //       const ctx = new context.Context("api-fxtrade.oanda.com", 443, true, "argomarketminer");
      //       ctx.setToken(req.body.token);
      //       const accSpec = new account.EntitySpec(ctx);
      //       const responseHandler = (res: context.Response) => {
      //             processApi("getAccount", null, res.rawBody, res);
      //       };
      //       accSpec.get(req.body.accountId, responseHandler);
   } else {
